...
def _verify_parameter_study(parameter_study: xarray.Dataset):
    ...
    if file_hashes != calculated_hashes:
        raise RuntimeError(
            f"Error opening {parameter_study_file}. File set hashes '{file_hashes}' not equal to "
            f"calculated content set hashes '{calculated_hashes}'. Was the parameter study file modified by hand?"
            "Was the parameter study file generated by an older version of WAVES?"
        )
