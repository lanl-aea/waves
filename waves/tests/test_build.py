...
def test_build():
    ...
    with patch('waves._utilities.tee_subprocess', return_value=(1, "some error message")) as mock_tee_subprocess, \
         patch("pathlib.Path.mkdir") as mock_mkdir, \
         pytest.raises(RuntimeError):
        try:
            _build.main(["dummy.target"], git_clone_directory='dummy/clone')
    ...
    max_iterations = 2
    with patch('waves._utilities.tee_subprocess', return_value=(0, "some other message")) as mock_tee_subprocess, \
         patch("pathlib.Path.mkdir") as mock_mkdir, \
         pytest.raises(RuntimeError):
        try:
    ...
    # subprocess call.
    with patch('waves._utilities.tee_subprocess', return_value=(1, "scons error message")) as mock_tee_subprocess, \
         patch("pathlib.Path.mkdir") as mock_mkdir, \
         pytest.raises(RuntimeError):
        try:
            _build.main(["dummy.target"])
