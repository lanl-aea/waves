...
class AbaqusPseudoBuilder:
    ...
    Parameters
    ----------
    abaqus_builder : SCons Builder
        An Builder generated by `waves.scons_extensions.abaqus_solver_builder_factory()`
    override_cpus : int, optional
        Override the task-specific default number of CPUs. This kwarg value is most useful if propagated from a
        user-specified option at execution time. If None, Abaqus Pseudo-Builder tasks will use the task-specific
        default.
    """
    def __init__(self, abaqus_builder, override_cpus=None):
        self.abaqus_builder = abaqus_builder
    ...
    def __call__(self, env, job, inp=None, user=None, cpus=1, oldjob=None, write_restart=False, double='both',
        ...
        write_restart : bool, optional
            If True, add restart files to target list. This is required if you want to use these restart files for a restart
            job.
        extra_sources : list, optional
            Additional sources to supply to builder.
        ...
        **builder_kwargs : kwargs, optional
        ...
        Returns
        -------
        target : list of SCons Target
            All targets associated with Abaqus simulation.

        Example
        -------
        To use ``AbaqusPseudoBuilder``, first import it in your SConstruct file:

        .. code-block:: python

            from waves.scons_extensions import AbaqusPseudoBuilder

        Next, add the method to your environment:

        .. code-block:: python

            env.AddMethod(
                AbaqusPseudoBuilder(abaqus_builder=env.AbaqusSolver, override_cpus=env.GetOption("solve_cpus")),
                "Abaqus")

        In the above example, it is assumed that `env.AbaqusSolver` is a Builder generated by
        `waves.scons_extensions.abaqus_solver_builder_factory` and that a commandline option will allow the user to
        override the number of CPUs.
        ...
        if inp:
            ...
            inp = pathlib.Path(job).with_suffix('.inp')
