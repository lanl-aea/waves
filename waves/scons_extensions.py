    ...
    def __call__(
        ...
        **kwargs,
        ...
        :param num_oldjob_restart_files: Number of MPI process-specific restart files generated by ``oldjob``.
            ...
            a .mdl file, but will produce .mdl.0 and .mdl.1 files. When restarting/importing that Abaqus Standard job,
            specify ``num_oldjob_restart_files = 2``. Abaqus Explicit restart files are not affected by the number of
            MPI processes and ``num_oldjob_restart_files = 1`` should be specified when restarting/importing from an
            Abaqus Explicit job. If ``oldjob`` is a single Abaqus job, specify a single integer. If ``oldjob`` is a list
            of Abaqus jobs, specify a list of integers for job-specific restart files or a single integer to be applied
            to all old jobs.
