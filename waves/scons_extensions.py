...
def ssh_builder_actions(
    ...
    r"""Wrap and modify a builder's action list with remote copy operations and SSH commands.
...
class AbaqusPseudoBuilder:
    ...
    def __call__(
        ...
        num_oldjob_restart_files: list[int] | int = 1,
        ...
        **kwargs,
        ...
        :param cpus: CPUs to use for simulation. Is superceded by ``override_cpus`` if provided during object
            ...
            trigger a rebuild.
        :param processes: Number of MPI processes. This option is escaped in the action string, i.e. changing the
        ...
        :param num_oldjob_restart_files: Number of MPI process-specific restart files generated by ``oldjob``.
            For Abaqus Standard jobs, the number of restart files (specifically .mdl and .stt) will be determined by the
            number of MPI processes the job used. For example, an Abaqus Standard job on 2 MPI processes will not output
            a ``.mdl`` file, but will produce ``.mdl.0`` and ``.mdl.1`` files. When restarting/importing that Abaqus
            Standard job, specify ``num_oldjob_restart_files = 2``. Abaqus Explicit restart files are not affected by
            the number of MPI processes and ``num_oldjob_restart_files = 1`` should be specified when
            restarting/importing from an Abaqus Explicit job. If ``oldjob`` is a single Abaqus job, specify a single
            integer. If ``oldjob`` is a list of Abaqus jobs, specify a list of integers for job-specific restart files
            or a single integer to be applied to all old jobs.
        ...
        if oldjob:
            ...
            if isinstance(num_oldjob_restart_files, int):
                # If int, apply to all old jobs
                num_oldjob_restart_files = [num_oldjob_restart_files] * len(oldjob)
            # Check num_oldjob_restart_files is same length as oldjob
...
class WAVESEnvironment(SConsEnvironment):
    """Overload SConsEnvironment with WAVES construction environment methods and builders."""

    def __init__(
        self,
        *args,
        ABAQUS_PROGRAM: str = "abaqus",  # noqa: N803
        ANSYS_PROGRAM: str = "ansys",  # noqa: N803
        CCX_PROGRAM: str = "ccx",  # noqa: N803
        CHARMRUN_PROGRAM: str = "charmrun",  # noqa: N803
        FIERRO_EXPLICIT_PROGRAM: str = "fierro-parallel-explicit",  # noqa: N803
        FIERRO_IMPLICIT_PROGRAM: str = "fierro-parallel-implicit",  # noqa: N803
        INCITER_PROGRAM: str = "inciter",  # noqa: N803
        MPIRUN_PROGRAM: str = "mpirun",  # noqa: N803
