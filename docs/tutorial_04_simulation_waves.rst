...
:meth:`waves.scons.abaqus_journal` builder in the code pertaining to ``# Mesh``.
...
``job_name`` is used in the Abaqus solver call. See the :meth:`waves.scons.abaqus_solver` API for information about
...
simulation with the :meth:`waves.scons.abaqus_solver` builder. The ``target`` list includes only the
``{job_name}.sta`` file, though many more output files will be generated by the Abaqus solver. As discussed in the
:meth:`waves.scons.abaqus_solver` API, there is a default list of targets that will be appended to the ``target``
...
.. note::
    ...
    The :meth:`waves.scons.solve_abaqus` builder has the option to retrieve non-zero exit codes from the Abaqus
    solver by parsing the output ``.sta`` file using ``grep``. The :meth:`waves.scons.solve_abaqus` API provides an
