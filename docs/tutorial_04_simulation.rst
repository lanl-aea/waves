...
simulation with the :meth:`waves.scons_extensions.abaqus_solver_builder_factory` builder. The ``target`` list includes
only the ``{job_name}.sta`` file, though many more output files will be generated by the Abaqus solver. As discussed in
the :meth:`waves.scons_extensions.abaqus_solver_builder_factory` API, there is a default list of targets that will be
appended to the ``target`` list by the builder. The ``source`` list once again utlizes the existing
``solve_source_list`` variable, and the ``job_name`` now uses the ``job_name`` variable that was defined at the
beginning of this tutorial. Lastly, the ``-double both`` option is included with the ``abaqus_options`` variable.
